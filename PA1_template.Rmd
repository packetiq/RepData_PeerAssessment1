...
Interval `r maxInterval` contains the maximum number of steps (`r as.integer(maxStepsEntry$Steps)`)    
...
```
The total number of missing values in the dataset is `r totMissing`  
...
The normalized mean number of steps taken each day is `r newMeanSteps`.  
The normalized median number of steps taken each day is `r newMedianSteps`.  

The difference between the original and 'normalized' values is noticable but not significant. The mean value for the original dataset was `r meanSteps` as compared to the normalized mean of `r newMeanSteps`, which is also reflected as a slight change in the middle of the histogram. In addition, the 0 -to- 1000 Steps/Day count column in the 'normalized' histogram decreased considerably due to the elimination of a large number of missing values.
